# -*- coding: utf-8 -*-
"""
Created on Sat Sep 20 11:05:24 2025
...
sntnc = nltk.sent_tokenize(paragraph)

corpus=[]

for i in range(len(sntnc)):
    review = re.sub('[^a-zA-Z]', ' ', sntnc[i])
    review = review.lower()
    review = review.split()
#   review = [ps.stem(word) for word in review if not word in set(stopwords.words('english'))]
    review = [wrd.lemmatize(word) for word in review if not word in set(stopwords.words('english'))]  
    review = ' '.join(review)
